        ...
        goldsky_workers.emplace_back([this](){
            ...
            while(goldsky_queue.wait_and_pop(task)){
                ...
                if(!open_positions.empty()){
                    gamma_queue.push({task.user_id, realized, total_positions_count, std::move(open_positions)});
                }
                else{
        ...
        gamma_workers.emplace_back([this](){
            ...
            while(gamma_queue.wait_and_pop(task)){
                ...
                if(!unresolved_positions.empty()){
                    clob_queue.push({task.user_id, task.realized_pnl, resolved_unrealized,task.total_positions, std::move(unresolved_positions)});
                }
                else{
